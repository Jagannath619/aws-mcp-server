"""AWS Application Load Balancer MCP server."""
import asyncio
import json
import logging
import os
...
def _handle_boto_error(error: Exception) -> ToolError:
    logger.exception("AWS ALB operation failed: %s", error)
...
@server.tool("list_load_balancers", "List Application Load Balancers")
...
@server.tool("describe_load_balancer", "Describe a load balancer")
...
@server.tool("create_load_balancer", "Create an Application Load Balancer")
async def create_load_balancer(name: str, subnets: List[str], security_groups: Optional[List[str]] = None, scheme: str = "internet-facing", ip_address_type: str = "ipv4") -> List[Dict[str, Any]]:
    try:
        params: Dict[str, Any] = {
            "Name": name,
            "Subnets": subnets,
...
@server.tool("delete_load_balancer", "Delete an Application Load Balancer")
...
@server.tool("modify_load_balancer_attributes", "Update ALB attributes")
...
@server.tool("list_target_groups", "List target groups")
...
@server.tool("create_target_group", "Create a target group")
async def create_target_group(name: str, protocol: str, port: int, vpc_id: str, target_type: str = "instance", health_check_path: Optional[str] = None, health_check_protocol: Optional[str] = None) -> List[Dict[str, Any]]:
    try:
        params: Dict[str, Any] = {
            "Name": name,
            "Protocol": protocol,
        ...
        if health_check_path:
            params["HealthCheckPath"] = health_check_path
...
@server.tool("delete_target_group", "Delete a target group")
...
@server.tool("register_targets", "Register targets to a target group")
...
@server.tool("deregister_targets", "Deregister targets from a target group")
...
@server.tool("list_listeners", "List listeners for a load balancer")
...
@server.tool("create_listener", "Create a listener for a load balancer")
async def create_listener(load_balancer_arn: str, protocol: str, port: int, default_actions: List[Dict[str, Any]], ssl_policy: Optional[str] = None, certificates: Optional[List[Dict[str, str]]] = None) -> List[Dict[str, Any]]:
    try:
        params: Dict[str, Any] = {
...
@server.tool("delete_listener", "Delete a listener")
...
@server.tool("modify_listener", "Modify listener configuration")
async def modify_listener(listener_arn: str, default_actions: Optional[List[Dict[str, Any]]] = None, certificates: Optional[List[Dict[str, str]]] = None, ssl_policy: Optional[str] = None, port: Optional[int] = None, protocol: Optional[str] = None) -> List[Dict[str, Any]]:
    try:
